...
func main() {
	...
	listenF := flag.String("l", "", "wait for incoming connections on VSOCK")
	...
	if *listenF == "" {
		log.Fatal("Please provide a VSOCK multiaddr to bind on with -l")
	}
...
func makeBasicHost(listenAddr string, insecure bool, randseed int64) (host.Host, error) {
	var r io.Reader
	if randseed == 0 {
		r = rand.Reader
	} else {
...
func startListener(ctx context.Context, ha host.Host, listenAddr string, insecure bool) {
	...
	vsockMaddr, err := ma.NewMultiaddr(listenAddr)
	if err != nil {
		panic(err)
	}
